...
OUTPUT_DIR = 'train'
DATA_DIR ='data'
alphabet = ' ﺀآابپتثجچحخدذرزژسشصضطظعغفقکگلمنوهﯓی‬ٌ ‬ًّ ‬َ ‬ِ ‬ُ ‬ْ ‬۰۱۲۳۴۵۶۷۸۹'
...
def is_valid_str(in_str):
    search = re.compile(r'^[\u0600-\u06FF\uFB8A\u067E\u0686\u06AF\u200C\u200F ]+$').search        
...
def ctc_lambda_func(args):
    ...
    #ctc_batch_loss = tf.keras.backend.ctc_batch_cost(labels,y_pred,input_length,label_length)
...
class TextImageGenerator(keras.callbacks.Callback):
    ...
    def build_word_list(self, num_words, max_string_len=None, mono_fraction=0.5):
        ...
        with open('data/monopers.txt', 'rt',encoding='utf-8') as f:
            for line in f:
        ...
        line=""
        tmp_string_list = []
        filename = os.path.join(fileDir, 'data/monopers.txt')
        with open('data/bipers2.txt','rt',encoding='utf-8') as f:
            for line in f:
        ...
        if len(tmp_string_list) != self.num_words:
            raise IOError('Could not pull enough words from supplied monogram and bigram files. '+str(len(tmp_string_list)) )
    ...
    def on_train_begin(self, logs={}):
        self.build_word_list(1600, 16, 0.5) #chops to change as para
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if epoch >= 21 and self.max_string_len < 12:
            self.build_word_list(3200, 16, 0.5)
...
class ThaanaOCR:
    def __init__(self, img_w=512, labeltype_hinting=True, verbose=1):
        ...
        fdir = DATA_DIR
        self.img_gen = TextImageGenerator(monogram_file=os.path.join(fdir, 'monopers.txt'),
                                     bigram_file=os.path.join(fdir, 'bipers2.txt'),
                                     minibatch_size=32,
                                     img_w=img_w,
                                     img_h=self.img_h,
